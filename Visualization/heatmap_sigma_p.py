#!/usr/bin/env python3
"""
Genera un Heatmap 2D de la dispersión espacial promedio <sigma_v> en función de p y K.
Incluye un sistema de caché para evitar reprocesar los datos crudos.
"""
...
CACHE_FILE = SCRIPT_DIR / "cache_dispersion_2d.csv"
T_EST = 200.0


def parse_single_file(filepath: Path) -> tuple[float, float, float] | None:
    k_val = None
    p_val = None
    sigmas_temporal = []
    current_t = -1.0
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line.startswith("K="):
                    ...
                    current_t = float(line[14:])
                elif line.startswith("v "):
                    if current_t >= T_EST:
                        v_vals = np.fromstring(line[2:], sep=" ")
                        if v_vals.size > 0:
                            sigmas_temporal.append(v_vals.std())

        if k_val is None or p_val is None or not sigmas_temporal:
            return None

        return k_val, p_val, float(np.mean(sigmas_temporal))
...
def main():
    if CACHE_FILE.exists():
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = {executor.submit(parse_single_file, Path(f)): f for f in files}
        ...
        df_raw = pd.DataFrame(raw_data, columns=["K", "p", "sigma"])
        
        print("Promediando realizaciones independientes...")
        df_grid = df_raw.groupby(["K", "p"])["sigma"].mean().reset_index()
        
        df_grid.to_csv(CACHE_FILE, index=False)
        print(f"Caché guardado en: {CACHE_FILE}")

    pivot_df = df_grid.pivot(index="K", columns="p", values="sigma")
    ...
    mesh = ax.pcolormesh(X, Y, Z, cmap="viridis", shading="nearest")
    
    cbar = fig.colorbar(mesh, ax=ax)
    cbar.set_label(r"Dispersión espacial promedio $\langle\sigma_v\rangle$", fontsize=14)
