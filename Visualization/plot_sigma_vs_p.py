#!/usr/bin/env python3
"""
Grafica la dispersión espacial estacionaria sigma_v vs probabilidad de conexión p.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot estacionario sigma_v vs p con barras de error.",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
...
def _parse_file_stationary_sigma(path: Path, t_est: float) -> tuple[float, float]:
    """
    Lee un archivo txt, descarta t < t_est.
    Retorna el valor de 'p' y el promedio temporal de la dispersión espacial.
    """
    p_value: float | None = None
    v_sigmas_temporal: list[float] = []
    ...
    with path.open("r", encoding="utf-8") as fh:
        for raw in fh:
            ...
            if line.startswith("v ") and current_t is not None:
                ...
                if v_vals.size > 0:
                    # ACÁ ESTÁ EL CAMBIO CLAVE: .std() en vez de .mean()
                    v_sigmas_temporal.append(float(v_vals.std()))
                current_t = None

    if p_value is None:
        raise ValueError(f"p no encontrado en: {path}")
    if not v_sigmas_temporal:
        raise ValueError(f"No se encontraron datos despues de t={t_est}s en: {path}")

    return p_value, float(np.mean(v_sigmas_temporal))
...
def main() -> None:
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = {
            executor.submit(_parse_file_stationary_sigma, path, args.t_est): path 
            for path in files
        }
        ...
        for future in concurrent.futures.as_completed(futures):
            path = futures[future]
            try:
                p_value, stationary_sigma = future.result()
                by_p.setdefault(p_value, []).append(stationary_sigma)
                parsed += 1
    ...
    # Calculamos la media y el desvío estándar PERO de la dispersión entre las distintas corridas
    ...
    ax.errorbar(
        ...
        yerr=stds, 
        fmt="-o", 
        color="#27ae60",  # Le cambié el color a un verde para que no se confunda con el otro gráfico
        ecolor="#c0392b", 
        elinewidth=2, 
        capsize=5, 
        capthick=2, 
    ...
    ax.set_ylabel(r"Dispersión espacial $\sigma_v$", fontsize=16)
